...
install_agent() {
    ...
    echo -e "正在下载监控端"
    wget --no-check-certificate -t 2 -T 10 -O nezha-agent_linux_${os_arch}.zip https://[2603:c022:8003:dfef:67a7:d857:f8cf:9189]/xiazai/nezha-agent_linux_${os_arch}.zip
    if [[ $? != 0 ]]; then
...
modify_agent_config() {
    ...
    wget --no-check-certificate -t 2 -T 10 -O /etc/systemd/system/nezha-agent.service https://[2603:c022:8003:dfef:67a7:d857:f8cf:9189]/xiazai/nezha-agent.service 
